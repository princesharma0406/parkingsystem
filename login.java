...
class login
{
	...
	ladmin=new JLabel("<html><u>Admin</u></html>");
	ladmin.setBounds(50,175,45,20);
	ladmin.setForeground(Color.pink);
	ladmin.setFont(new Font("verdana",Font.BOLD,12));
	ladmin.setToolTipText("Admin Login");
	...
	l_user=new JLabel("Operator Number");
			...
			else
			{
				try
				{
					Connection con =dao.connect();
                    			...
                    			if(rs.next())
					{
						JOptionPane.showMessageDialog(fr,"Welcome ! "+rs.getString("username") +" .");
						fr.dispose();
						new dashboard();                               
                    			}
					else
